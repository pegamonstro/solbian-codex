...
Three surfaces over local SQLite stores (Python stdlib only; no frameworks):
...
* **Engine** — strictly READ-ONLY browse of the Stage 5 engine store
  (``proposal`` / ``provenance_link`` / ``relationship`` / ``analysis_run``).
  A "Run engine (dry-run)" button shells to the Stage 5 engine on a throwaway
  copy with ``--dry-run`` and commits nothing.
...
* The engine store is opened with SQLite ``mode=ro``; the server never writes it.
  Even the dry-run runs against a temporary copy.
...
# Stage 5 engine store + CLI (read-only for this UI unless the dry-run button is
# used, and even then only a temp copy is written).
STAGE5_DIR = "/Users/archcore/solbian/codex-phase-i-stage5"
DEFAULT_ENGINE_DB = os.environ.get(
    "SOLBIAN_ENGINE_DB", os.path.join(STAGE5_DIR, "data", "codex_phase_i.sqlite"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802
        ...
        if path.startswith("/api/engine/"):
            self._error(405, "the Engine surface is read-only; only /api/engine/dry-run is accepted")
            return
        if path == "/api/solace/conversations":
    ...
    def _reject_mutation(self):
        ...
        if path.startswith("/api/codex/"):
            ...
            self._error(405, "the Engine surface is read-only; only POST /api/engine/dry-run is accepted")
        else:
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Stage 4 three-surface prototype server")
    ...
    p.add_argument("--engine-db", default=DEFAULT_ENGINE_DB,
                   help="Stage 5 engine store to browse read-only")
    p.add_argument("--engine-py", default=DEFAULT_ENGINE_PY,
                   help="Stage 5 engine CLI used by the dry-run button")
...
def main(argv=None) -> int:
    ...
    print(f"[app] Codex Solbian — Stage 4 three-surface prototype")
    print(f"[app] db        : {httpd.db_path}")
    print(f"[app] engine db : {httpd.engine_db_path}")
    print(f"[app] url       : http://{host}:{port}/")
    print(f"[app] CANONICAL: NONE · Solace writes conversation/message only · "
          f"Codex + Engine read-only")
    sys.stdout.flush()
    try:
        httpd.serve_forever()
