...
Boots ``app.py`` on an ephemeral port against throwaway copies of the working
store and the Stage 5 engine store, exercises the Solace, Codex and Engine
endpoints over real HTTP, asserts the read-only guarantees, then kills the
server. Exits 0 only if every check passes.
...
ENGINE_DB = "/Users/archcore/solbian/codex-phase-i-stage5/data/codex_phase_i.sqlite"

RESULTS: list[tuple[str, bool, str]] = []
SOURCE_SNAPSHOT: list = []
ENGINE_SNAPSHOT: list = []
...
def main() -> int:
    global SOURCE_SNAPSHOT, ENGINE_SNAPSHOT
    ...
    engine_tmp = None
    engine_source = ENGINE_DB if os.path.exists(ENGINE_DB) else None
    if engine_source:
        engine_tmp = os.path.join(tmpdir, "engine.sqlite")
        shutil.copy2(engine_source, engine_tmp)
        ENGINE_SNAPSHOT = snapshot_proposals(engine_tmp)
    else:
        print(f"NOTE  Stage 5 engine store not found: {ENGINE_DB}; skipping Engine checks")
    ...
    cmd = [sys.executable, APP, "--port", str(port), "--db", tmp_db, "--no-setup"]
    if engine_tmp:
        cmd += ["--engine-db", engine_tmp]
    ...
    try:
        ...
        if up:
            run_checks(base, tmp_db, engine_tmp)
...
def run_checks(base: str, tmp_db: str, engine_tmp: str | None) -> None:
    # ---- static shell ---------------------------------------------------
    ...
    # ---- engine (read-only proposals) ----------------------------------
    if engine_tmp:
        run_engine_checks(base)
    ...
    # ---- engine store must be untouched ---------------------------------
    if engine_tmp:
        check("engine store proposal rows were not mutated by any request",
              snapshot_proposals(engine_tmp) == ENGINE_SNAPSHOT,
              f"before={len(ENGINE_SNAPSHOT)} after={len(snapshot_proposals(engine_tmp))}")
...
def run_engine_checks(base: str) -> None:
    """Engine tab: read-only list/detail of Stage 5 proposals + safe dry-run."""
    ...
    if isinstance(emeta, dict):
        ...
        check("engine proposal statuses are PROPOSED/WORKING only",
              set(emeta.get("proposal_statuses") or []) <= {"PROPOSED", "WORKING"},
              str(emeta.get("proposal_statuses")))
    ...
    status, listing = jget(base + "/api/engine/proposals?limit=5")
    ...
    if sample:
        ...
        resolved = [p for p in ((prop or {}).get("provenance") or [])
                    if p.get("source_kind") == "source_document" and p.get("source_rel_path")]
        check("engine provenance resolves to a source_document path",
              len(resolved) >= 1, f"resolved={len(resolved)}")
        check("engine proposal detail exposes based_on",
              bool(prop) and isinstance(prop.get("based_on"), list))

    # non-dry-run mutations are refused
