...
| Ref | Meaning |
|-----|---------|
| `main` | Current product tip (historical Mac-tracked checkout) |
...
- **Gitea** `00_codex_solbian/solbian.git` → lab forge; archive import pending/RO

## Operator rules
1. Do not `git merge archive/*` or `working/*` into `main`.
2. To inspect history: `git switch archive/…` or `git show tag`.
3. Retiring sibling remotes: **archive on GitHub first**; hard-delete only after ARCH backup verify.
4. CANONICAL elevation is a JD decision — git layout ≠ doctrine.
